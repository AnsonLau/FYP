       ...
       <%
        ...
        <div id="content" class="SITE_STRUCTURE content">
            <div style="display: block;" class="tabs_item" id="select_main">
                <div id="message">
                    <h1 align="center">User Account Administrate</h1>
                    <%
                        ArrayList<UserBean> bean;

                        if (request.getAttribute("accounts") != null) {
                            bean = (ArrayList<UserBean>) request.getAttribute("accounts");
                            out.println(bean.size() + " user(s) waiting for authenticate");
                            if (bean.size() != 0) {
                                ...
                                out.println("<tr><th>User ID</th><th>Username</th><th>User Password</th><th>MemberID</th><th>StaffID</th><th>First Name</th><th>Last Name</th><th>Gender</th><th>Tel</th><th>Name In Chinese</th><th>Email</th><th>Validation<input type='checkbox' id='selectAll'/></th></tr>");
                                for (int i = 0; i < bean.size(); i++) {
                                    out.println("<tr><td>" + bean.get(i).getUserID() + "</td>");
                                    out.println("<td>" + bean.get(i).getUserName() + "</td>");
                                    out.println("<td>" + bean.get(i).getPassword() + "</td>");
                                    out.println("<td>" + bean.get(i).getMemberID() + "</td>");
                                    out.println("<td>" + bean.get(i).getStaffID() + "</td>");
                                    ...
                                    out.println("<td><input type='checkbox' class='checkbox1' name='ValidationID' value='" + bean.get(i).getIsAuthenticated() + "'/></td>");
                                }
                                out.println("</table>");
